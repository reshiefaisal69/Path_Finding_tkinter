...
def yens_k_shortest_path(grid, tickTime):
...
def StartAlgorithm():
    ...
    elif algMenu.get() == 'Yen\'s Algorithm':
        yens_k_shortest_path(grid, speedScale.get())
...
def random_walls(grid):
    ...
    if not Spot.start_point:
        current = grid[random.randint(
            0, ROWS - 1)][random.randint(0, ROWS - 1)]
...
algMenu = ttk.Combobox(UI_frame, textvariable=selected_alg,
                       values=['A-star Algorithm', 'Breadth-First Algorithm', 'Depth-First Algorithm', 'Yen\'s Algorithm', 'Dijkstra\'s Algorithm'], font=font)
